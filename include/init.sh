...
PHP_Make_Install()
{
    if echo "${Php_Ver}" | grep -Eqi '^php-8\.' || echo "${php_version}" | grep -Eqi '^8\.'; then
        PHP_Make_Extra_Options=""
    else
...
Install_Freetype()
{
    if [ "${LNMP_FORCE_NEW_FREETYPE:-n}" = "y" ] || echo "${Ubuntu_Version}" | grep -Eqi "^1[89]\.|2[0-9]\." || echo "${Mint_Version}" | grep -Eqi "^19|2[0-9]" || echo "${Deepin_Version}" | grep -Eqi "^15\.[7-9]|15.1[0-9]|1[6-9]|2[0-9]" || echo "${Debian_Version}" | grep -Eqi "^9|1[0-9]" || echo "${Raspbian_Version}" | grep -Eqi "^9|1[0-9]" || echo "${Kali_Version}" | grep -Eqi "^202[0-9]" || echo "${UOS_Version}" | grep -Eqi "^2[0-9]" || echo "${CentOS_Version}" | grep -Eqi "^(8|9|10)" || echo "${RHEL_Version}" | grep -Eqi "^(8|9|10)" || echo "${Oracle_Version}" | grep -Eqi "^(8|9|10)" || echo "${Fedora_Version}" | grep -Eqi "^3[0-9]|29" || echo "${Rocky_Version}" | grep -Eqi "^(8|9|10)" || echo "${Alma_Version}" | grep -Eqi "^(8|9|10)" || echo "${openEuler_Version}" | grep -Eqi "^2[0-9]" || echo "${Anolis_Version}" | grep -Eqi "^(8|9|10|2[0-9])" || echo "${Kylin_Version}" | grep -Eqi "^V1[0-9]" || echo "${Amazon_Version}" | grep -Eqi "^202[3-9]" || echo "${OpenCloudOS_Version}" | grep -Eqi "^(8|9|10|23)" || echo "${HCE_Version}" | grep -Eqi "^2\.[0-9]"; then
        ...
        if [ "${LNMP_FREETYPE_DISABLE_HARFBUZZ:-n}" = "y" ]; then
            ./configure --prefix=/usr/local/freetype --enable-freetype-config --without-harfbuzz
        else
...
EOF
    ...
    cd ${cur_dir}/src/
    rm -rf ${cur_dir}/src/${Freetype_Ver} ${cur_dir}/src/${Freetype_New_Ver}
}
...
Install_Libzip()
{
    ...
    # Modern distributions do not always provide libzip-devel in their
    # enabled repositories. Test what PHP's configure actually needs instead
    # of assuming every non-EL7 system package installation succeeded.
    if pkg-config --exists 'libzip >= 0.11 libzip != 1.3.1 libzip != 1.7.0'; then
        Echo_Green "libzip ... ok ($(pkg-config --modversion libzip))"
        return 0
    fi
    ...
    if ! pkg-config --exists 'libzip >= 0.11 libzip != 1.3.1 libzip != 1.7.0'; then
        Echo_Red "libzip was installed, but pkg-config still cannot find a supported version."
        Echo_Red "PKG_CONFIG_PATH=${PKG_CONFIG_PATH}"
        exit 1
    fi
