...
{
    local Configure_Arg
    local Configure_CPPFLAGS="${CPPFLAGS:-}"
    ...
    for Configure_Arg in "$@"; do
        if [ "${LNMP_USE_SYSTEM_FREETYPE:-n}" = "y" ] && [ "${Configure_Arg}" = "--with-freetype=/usr/local/freetype" ]; then
            Configure_Arg="--with-freetype"
        fi
        if [ "${Configure_Arg}" = "--with-iconv" ]; then
            Configure_Use_System_Iconv="y"
            if [ -n "${Configure_CPPFLAGS}" ]; then
                Configure_CPPFLAGS="-DLIBICONV_PLUG ${Configure_CPPFLAGS}"
            else
                Configure_CPPFLAGS="-DLIBICONV_PLUG"
            fi
        fi
    ...
    echo "========================== PHP configure command =========================="
    if [ "${Configure_Use_System_Iconv}" = "y" ]; then
        printf 'CPPFLAGS=%q ' "${Configure_CPPFLAGS}"
    fi
    printf '%q ' "$@"
    printf '\n'
    echo "============================================================================"
    ...
    if [ ${Configure_Status} -ne 0 ]; then
        if [ -s config.log ]; then
            \cp config.log "${Configure_Error_Log}"
            Echo_Red "PHP configure failed. Full config.log saved to ${Configure_Error_Log}"
        else
            Echo_Red "PHP configure failed with exit code ${Configure_Status}; config.log was not generated."
        fi
        # Stop immediately. Continuing to make/cleanup would hide the real
        # configure error and remove the source tree containing config.log.
...
Install_Composer()
{
    ...
    local Composer_Is_Legacy="n"

    if echo "${PHPSelect}" | grep -Eqi '^[1-8]$' || echo "${php_version}" | grep -Eqi '^5.[2-6].*|7.[0-2].*' || echo "${Php_Ver}" | grep -Eqi "php-5.[2-6].*|php-7.[0-2].*"; then
        Composer_Is_Legacy="y"
    fi

    if [ "${Composer_Is_Legacy}" = "y" ]; then
        [ -s "${cur_dir}/src/composer-2.2.phar" ] && Composer_Local_File="${cur_dir}/src/composer-2.2.phar"
    elif [ -s "${cur_dir}/src/composer.phar" ]; then
        Composer_Local_File="${cur_dir}/src/composer.phar"
    elif [ -s "${cur_dir}/src/composer-2.2.phar" ]; then
        Composer_Local_File="${cur_dir}/src/composer-2.2.phar"
    fi

    if [ -n "${Composer_Local_File}" ] && [ -s "${Composer_Local_File}" ]; then
        echo "${Composer_Local_File} [found], installing Composer from local file..."
        if cp "${Composer_Local_File}" /usr/local/bin/composer && chmod +x /usr/local/bin/composer; then
            echo "Composer install successfully from local file."
            return 0
        fi
        Echo_Red "Composer local file install failed: ${Composer_Local_File}"
        return 1
    fi

    if [ "${CheckMirror}" != "n" ]; then
        echo "Downloading Composer..."
        local Composer_Wget_TLS_Option=""
        [ "${Download_Insecure}" = "y" ] && Composer_Wget_TLS_Option="--no-check-certificate"
        if [ "${Composer_Is_Legacy}" = "y" ]; then
            wget --progress=dot:giga --prefer-family=IPv4 ${Composer_Wget_TLS_Option} -T 120 -t3 ${Download_Mirror}/web/php/composer/composer-2.2.phar -O /usr/local/bin/composer
            if [ $? -eq 0 ]; then
            ...
            wget --progress=dot:giga --prefer-family=IPv4 ${Composer_Wget_TLS_Option} -T 120 -t3 https://mirrors.aliyun.com/composer/composer.phar -O /usr/local/bin/composer
            if [ $? -eq 0 ]; then
                echo "Composer install successfully."
                chmod +x /usr/local/bin/composer
            else
...
Install_PHP_52()
{
    ...
    ./buildconf --force
    if [ "${Stack}" = "lnmp" ]; then
        Run_Configure ./configure --prefix=/usr/local/php --with-config-file-path=/usr/local/php/etc --with-config-file-scan-dir=/usr/local/php/conf.d --with-mysql=${MySQL_Dir} --with-mysqli=${MySQL_Config} --with-pdo-mysql=${MySQL_Dir} --with-iconv-dir --with-freetype-dir=/usr/local/freetype --with-jpeg-dir --with-png-dir --with-zlib --with-libxml-dir=/usr --enable-xml --enable-discard-path --enable-magic-quotes --enable-safe-mode --enable-bcmath --enable-shmop --enable-sysvsem --enable-inline-optimization ${with_curl} --enable-mbregex --enable-fastcgi --enable-fpm --enable-force-cgi-redirect --enable-mbstring --with-mcrypt --enable-ftp --with-gd --enable-gd-native-ttf ${with_openssl} --with-mhash --enable-pcntl --enable-sockets --with-xmlrpc --enable-zip --enable-soap --with-gettext --with-mime-magic ${PHP_Buildin_Option} ${PHP_Modules_Options}
    else
        Run_Configure ./configure --prefix=/usr/local/php --with-config-file-path=/usr/local/php/etc --with-config-file-scan-dir=/usr/local/php/conf.d --with-apxs2=/usr/local/apache/bin/apxs --with-mysql=${MySQL_Dir} --with-mysqli=${MySQL_Config} --with-pdo-mysql=${MySQL_Dir} --with-iconv-dir --with-freetype-dir=/usr/local/freetype --with-jpeg-dir --with-png-dir --with-zlib --with-libxml-dir=/usr --enable-xml --enable-discard-path --enable-magic-quotes --enable-safe-mode --enable-bcmath --enable-shmop --enable-sysvsem --enable-inline-optimization ${with_curl} --enable-mbregex --enable-mbstring --with-mcrypt --enable-ftp --with-gd --enable-gd-native-ttf ${with_openssl} --with-mhash --enable-pcntl --enable-sockets --with-xmlrpc --enable-zip --enable-soap --with-gettext --with-mime-magic ${PHP_Buildin_Option} ${PHP_Modules_Options}
    fi
    PHP_Make_Install
...
Install_PHP_80()
{
    ...
    PHP_Openssl3_Patch
    if [ "${Stack}" = "lnmp" ]; then
        Run_Configure ./configure --prefix=/usr/local/php --with-config-file-path=/usr/local/php/etc --with-config-file-scan-dir=/usr/local/php/conf.d --enable-fpm --with-fpm-user=www --with-fpm-group=www --enable-mysqlnd --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --with-iconv --with-freetype=/usr/local/freetype --with-jpeg --with-zlib --enable-xml --disable-rpath --enable-bcmath --enable-shmop --enable-sysvsem ${with_curl} --enable-mbregex --enable-mbstring --enable-intl --enable-pcntl --enable-ftp --enable-gd ${with_openssl} --with-mhash --enable-pcntl --enable-sockets --with-zip --enable-soap --with-gettext ${with_fileinfo} --enable-opcache --with-xsl --with-pear --with-webp ${PHP_Buildin_Option} ${PHP_Modules_Options}
    else
        Run_Configure ./configure --prefix=/usr/local/php --with-config-file-path=/usr/local/php/etc --with-config-file-scan-dir=/usr/local/php/conf.d --with-apxs2=/usr/local/apache/bin/apxs --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --with-iconv --with-freetype=/usr/local/freetype --with-jpeg --with-zlib --enable-xml --disable-rpath --enable-bcmath --enable-shmop --enable-sysvsem ${with_curl} --enable-mbregex --enable-mbstring --enable-intl --enable-pcntl --enable-ftp --enable-gd ${with_openssl} --with-mhash --enable-pcntl --enable-sockets --with-zip --enable-soap --with-gettext ${with_fileinfo} --enable-opcache --with-xsl --with-pear --with-webp ${PHP_Buildin_Option} ${PHP_Modules_Options}
    fi
